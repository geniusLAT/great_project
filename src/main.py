...
def message_reply(message):
    try:
        ...
        if ud.check():
            bot.send_message(message.chat.id,str('Ваш запрос обрабатывается. Пожалуйста подождите.'))
            process(ud)
        else:
            bot.send_message(message.chat.id,str('Пожалуйста пришлите Вашу геопозицию, используя вложения Телеграм'))
    ...
    try:
        if message.location is not None:
            ...
            if ud.check():
                ...
                bot.send_message(message.chat.id,str('Что Вас интересует?'))
    except Exception as e:
        print(e)
